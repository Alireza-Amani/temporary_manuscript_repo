...
class PrepSVS:
    ...
    def __init__(
        ...
        self.mesh_param_file = MESHParameters(
            lyinfo_path=self.required_data.lyinfo_path,
    ...
    def create_soil_file(self):
        ...
        if self.verbose:
            ...
            message = (
                ...
                F"In this file {len(lyinfo_df['thickness_m'])} soil layers "
                F"are defined.\n"
            )
            print(message)
...
#     date_utc = dfmet.loc[:, col_names["utc_dtime"]].to_numpy()
    ...
    assert (np.all(shortwave_rad >= 0) and np.all(air_temp < 2000)), (
    ...
    assert (np.all(shumidity >= 0) and np.all(shumidity < 1)), (
...
def create_dtime_vars(dfmet, dtime_col_name: str):
    ...
    first_tstep = dfmet.loc[0, dtime_col_name]
